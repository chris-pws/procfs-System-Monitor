 ...
 */
string Process::Ram() { return lp::Ram(pid_); }
...
void Process::UpdateCpuData() {
    ...
    jiffies_proc_total =
        std::accumulate(jiffies_proc.begin(), jiffies_proc.end() - 1, 0);
    AddValue(kJiffiesProc_, jiffies_proc_total);

    cpu_util = 1000 *
               ((float)cpu_data_[kJiffiesProc_][lp::kPresent_] -
                (float)cpu_data_[kJiffiesProc_][lp::kPast_]) /
               ((float)cpu_data_[kJiffiesSys_][lp::kPresent_] -
                (float)cpu_data_[kJiffiesSys_][lp::kPast_]);
